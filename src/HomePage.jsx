      ...
      {activeCategoryDishes.map((dish) => (
        <div key={dish.id} className="flex justify-between w-[90%] m-auto py-5 border-b border-[#b2beb5] relative">
          <div className="phone:min-w-[50%]">
            ...
            <small className="block text-[#b2beb5] phone:max-w-[50%] phone:mt-6 tablet:max-w-[70%] sm:text-[14px] sm:max-w-[80%] md:text-[16px]">{dish.dish_description}</small>
            ...
            {dish.addonCat && dish.addonCat.length > 0 && <span className="text-sm text-red-600 phone:whitespace-nowrap">Customization Available</span>}
          </div>
          <div className="flex phone:absolute phone:right-0 md:relative">
            <span className="text-sm h-fit w-20 mt-10 mr-4 phone:text-[12px] phone:mr-1 md:text-[14px]">{dish.dish_calories} Calories</span>
            <img
